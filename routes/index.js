...
exports.api_songs = function(request, response) {
  ...
  let sql = ""
  var searchingTitle = true;
  if (urlObj.query['title']) {
    searchingTitle = true;
    title = `%${urlObj.query['title']}%`
    console.log("finding title: " + urlObj.query['title'])
    sql = "SELECT id, title FROM songs WHERE title LIKE '" + `%${urlObj.query['title']}%` +"'" +" OR composer LIKE '" + `%${urlObj.query['title']}%` +"' ORDER BY UPPER(title) LIMIT 30"
  }else if (urlObj.query['bookcode']) {
    searchingTitle = false;
    sql = "SELECT bookcode, title FROM bookcodes WHERE bookcode LIKE '" + `%${urlObj.query['bookcode']}%` +"' ORDER BY UPPER(bookcode) LIMIT 30"
  }
  ...
  db.all(sql, function(err, rows) {
    for (let i = 0; i < rows.length; i++) {
      ...
      //  '<a href="http://localhost:3000/songs">Visit W3Schools.com!</a>'
...
exports.api_songDetails = function(request, response) {
  ...
  let sql = "SELECT rowid, title, composer, bookcode, page, length, studentnum FROM songs WHERE id=?"
...
exports.api_bookDetails = function(request, response) {
  ...
  console.log("API: GET SONG DETAILS: " + bookcode)
  ...
  db.all(sql, bookcode, function(err, rows) {
    ...
    for (let i = 0; i < rows.length; i++) {
      result.bookcode = rows[0].bookcode
      result.title = rows[0].title
      result.format = rows[0].format
      result.filename = rows[0].filename
      result.page_offset = rows[0].page_offset
      result.num_pages = rows[0].num_pages
    }
    //write header with HTTP success code and MIME type
    response.writeHead(200, {
      'Content-Type': 'application/json'
